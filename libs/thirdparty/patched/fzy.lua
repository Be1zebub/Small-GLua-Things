-- https://github.com/Be1zebub/Small-GLua-Things/blob/master/libs/thirdparty/patched/fzy.lua
-- added compatibility with https://wiki.facepunch.com/gmod/utf8

-- The lua implementation of the fzy string matching algorithm

-- Copyright (c) 2020 Seth Warn (MIT)
-- upstream: https://github.com/swarn/fzy-lua/blob/main/src/fzy_lua.lua
...
function fzy.has_match(needle, haystack, case_sensitive)
  if not case_sensitive then
    needle = utf8.lower(needle)
    haystack = utf8.lower(haystack)
  end

  local j = 1
  for i = 1, utf8.len(needle) do
    j = string.find(haystack, utf8.sub(needle, i, i), j, true)
    if not j then
      return false
    else
      j = j + 1
...
local function precompute_bonus(haystack)
  ...
  local last_char = "/"
  for i = 1, utf8.len(haystack) do
    local this_char = utf8.sub(haystack, i, i)
...
local function compute(needle, haystack, D, M, case_sensitive)
  ...
  local n = utf8.len(needle)
  local m = utf8.len(haystack)
  ...
  for i = 1, m do
    haystack_chars[i] = utf8.sub(haystack, i, i)
  end
  ...
  for i = 1, n do
    ...
    local needle_char = utf8.sub(needle, i, i)
